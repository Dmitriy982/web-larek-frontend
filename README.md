...
interface: ILarekAPI 
getCardList: () => Promise<IProduct[]> Этот метод должен вернуть список лотов в виде массива объектов типа IProduct[].
orderProducts: (order: IOrder) => Promise<IOrderResult> Принимает объект заказа IOrder и размещает заказ, возвращая результат заказа типа IOrderResult.

interface: IPage
Определяет структуру данных, которые будут использоваться на странице.
Содержит три свойства:
counter: число, представляющее счетчик на странице.
catalog: массив элементов HTMLElement, представляющих список товаров на странице.
locked: логическое значение, указывающее, заблокирована ли страница.
