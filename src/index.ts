...
import {AuctionAPI} from "./components/common/LarekAPI";
...
events.on('card:select', (item: ProductItem) => {
    const preview = new Card('card', cloneTemplate(cardPreviewTemplate), {
        onClick: () => {events.emit('basket:submit', item),
            preview.buttonToggle = false;
        }
    });
...
events.on('basket:submit', (item: ProductItem) => {
    ...
    basket.items = appData.getcatalog().map(item => {
